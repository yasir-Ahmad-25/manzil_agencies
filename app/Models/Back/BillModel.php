...
class BillModel extends Model
{
    ...
    public function get_chargable_bills()
    {
        ...
        $sql = "SELECT MAX(created_date) as created_date, MAX(created_date) as bill_due_date, rental_id FROM tbl_rental_summary WHERE branch_id = 1 AND invoice_generated = 0 GROUP BY rental_id ORDER BY created_date DESC"; 

        // generated by AI
        // $sql = "SELECT MAX(rs.created_date) AS created_date, MAX(rs.created_date) AS bill_due_date, rs.rental_id, rs.paid FROM tbl_rental_summary rs WHERE rs.branch_id = $branch_id AND (rs.paid = 0 OR rs.paid IS NULL)"; 
    ...
    // public function get_chargable_tenants($rid)
    // {
    //     // If $rid is an array, ensure it is correctly formatted as a comma-separated list
    //     if (is_array($rid) && count($rid) > 0) {
    //         $rid = implode(',', array_map('intval', $rid)); // Convert each ID to an integer and implode them as a string
    //     } else {
    //         return []; // If no valid rental IDs are passed, return an empty array
    //     }
    
    //     // Drop temporary table if it exists
    //     $this->db->query("DROP TEMPORARY TABLE IF EXISTS tmp_tenant_bills");
    
    //     // Create temporary table with tenant details
    //     $sql = "CREATE TEMPORARY TABLE tmp_tenant_bills AS 
    //             SELECT r.rental_id, r.customer_id, tn.cust_name, r.ap_id, ap.ap_no, ap.price, r.end_date, s.owner_id, o.fullname  
    //             FROM tbl_rentals r
    //             JOIN tbl_customers tn ON tn.customer_id = r.customer_id
    //             JOIN tbl_apartments ap ON ap.ap_id = r.ap_id
    //             JOIN tbl_sites s ON s.site_id = ap.site_id
    //             JOIN tbl_owners o ON o.owner_id = s.owner_id  
    //             WHERE r.rental_id IN ($rid)
    //             AND r.end_date <= CURDATE()  
    //             AND r.rental_id NOT IN (SELECT rental_id FROM tbl_rental_summary WHERE paid_amount > 0)"; // Exclude tenants who have paid
    //     $this->db->query($sql);
    
    //     // Fetch the tenant details from the temporary table
    //     $sql = "SELECT * FROM tmp_tenant_bills";
    //     $query = $this->db->query($sql);
    ...
    public function updateInvoiceStatus($rental_id){
        $sql = "UPDATE tbl_rental_summary SET invoice_generated = 1 WHERE rental_id = $rental_id";
        $this->db->query($sql);
    }
